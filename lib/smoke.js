...
module.exports = function smoke() {
  ...
  Promise.all([wait(mainWin, 'main'), wait(miniWin, 'mini')]).then(async () => {
    try {
      ...
      await waitFor(mainWin, `db.tasks.some((t) => t.title.includes(${JSON.stringify(mark)}))`);
      ...
      if (!syncId) {
        console.log(`SYNC mini->main=${syncMain} main->mini=SKIPPED (không thấy việc vừa thêm để đổi tên)`);
        errs.push('probe: thêm việc ở mini mà app chính không cập nhật (đồng bộ 1 chiều hỏng)');
      } else {
      ...
      const plan = await mainWin.webContents.executeJavaScript(`(async () => {
        ...
        return { today, myday, navs, coO, truoc, sau, hien };
      })()`);
      ...
      const shoot = async (name) => {
        ...
        try {
          await w.webContents.executeJavaScript(`Promise.race([
            new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(() => setTimeout(r, 30)))),
            new Promise((r) => setTimeout(r, 400)),
          ])`);
      ...
      const modeShot = async (i, name, sel) => {
        ...
        for (let lan = 1; lan <= 3; lan++) {
          ...
          await mainWin.webContents.executeJavaScript('render()');   // ép vẽ lại khung rồi thử lại
        }
      };
      ...
      await new Promise((r) => setTimeout(r, 350));
      const hHelp = md5(await shoot('help'));
      await mainWin.webContents.executeJavaScript(`ui.helpTab = 'frame'; render()`);
      // capturePage() hay trả KHUNG CŨ -> ảnh tab 2 y hệt ảnh tab 1 dù DOM đã đổi. Chụp lại tới khi khác.
      let hFrame = hHelp;
      for (let lan = 1; lan <= 3 && hFrame === hHelp; lan++) {
        await new Promise((r) => setTimeout(r, 350 * lan));
        hFrame = md5(await shoot('help-frame'));
        if (hFrame === hHelp) await mainWin.webContents.executeJavaScript('render()');
      }
      console.log('HELP-SHOTS ' + JSON.stringify({ tab2KhacTab1: hFrame !== hHelp }));
      if (hFrame === hHelp) errs.push('probe: ảnh 2 tab Hướng dẫn y hệt nhau (tab 2 không vẽ ra hoặc capturePage trả khung cũ)');
