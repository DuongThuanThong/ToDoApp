...
module.exports = function smoke() {
  ...
  // CHỐT AN TOÀN: một promise treo (vd. requestAnimationFrame không cháy khi cửa sổ bị che) làm cả
  // lượt chạy đứng im vô hạn -> vòng lặp ngoài không bao giờ kết thúc. Hết giờ thì thoát và NÓI RÕ.
  const WATCHDOG_MS = 240000;
  const watchdog = setTimeout(() => {
    console.log('SMOKE HANG: quá ' + WATCHDOG_MS / 1000 + 's chưa xong — có chỗ bị treo (không phải lỗi app)');
  ...
  Promise.all([wait(mainWin, 'main'), wait(miniWin, 'mini')]).then(async () => {
    try {
      // MINI-BOOT: seedSmoke đặt settings.miniOn = true -> main.js phải TỰ bật cửa sổ mini lúc khởi động.
      // Đây chính là ca "tắt máy mở lại phải bấm tay mới có mini". Đo TRƯỚC khi smoke gọi showMini.
      const boot = { on: !!require('./store').get().settings.miniOn, exists: !!W.mini(), visible: false };
      try { boot.visible = !!(W.mini() && !W.mini().isDestroyed() && W.mini().isVisible()); } catch { /* chưa có cửa sổ */ }
      console.log('MINI-BOOT ' + JSON.stringify(boot));
      if (!boot.visible) errs.push('probe: mở app lên mà cửa sổ mini không tự bật (' + JSON.stringify(boot) + ')');
      W.showMini();
      ...
      if (tab < 30 || b1.width > W.TAB_MIN + 2 || b1.height > 56) errs.push('probe: thanh mép chưa gọn (' + b1.width + 'x' + b1.height + ', tab ' + tab + 'px)');
      ...
      if (peekBack > W.TAB_MIN + 2) errs.push('probe: hover-peek không thu lại');
      ...
      const tabW = () => W.TAB_MIN + 2;   // bề dày thanh mép: Windows ép tối thiểu TAB_MIN DIP (xem collapseMini)
      ...
      // TAB-VISIBLE: thanh mép là ĐƯỜNG DUY NHẤT mở lại mini bằng chuột -> phải thật sự VẼ RA và phải
      // nằm trong phần cửa sổ còn thấy được. Đã từng hỏng 2 kiểu: (1) tô bằng --primary mà accent của
      // người dùng ghi đè thành xám gần đen -> tàng hình; (2) thanh rộng 100% cửa sổ, mà cửa sổ bị
      // Windows ép rộng ~41 DIP trong khi vị trí tính theo tabSize=9 -> lòi ra ngoài màn hình.
      W.showMini();
      await new Promise((r) => setTimeout(r, 600));
      W.putAway();                       // y như người dùng bấm ra ngoài
      await waitFor(miniWin, `!!document.querySelector('.edge-tab')`, 3000);
      const tabv = await miniWin.webContents.executeJavaScript(`(() => {
        const el = document.querySelector('.edge-tab');
        if (!el) return { has: false, collapsed: !!ui.miniCollapsed };
        ...
        // Màu phải ăn theo HỆ THỐNG MÀU của app: dựng 1 ô tham chiếu cùng công thức token rồi so màu thật vẽ ra.
        const ref = document.createElement('div');
        ref.style.cssText = 'position:fixed;left:-999px;background:hsl(var(--secondary));color:hsl(var(--foreground));';
        document.body.appendChild(ref);
        const rcs = getComputedStyle(ref);
        const refBg = rgb(rcs.backgroundColor), refFg = rgb(rcs.color);
        ref.remove();
        const lechToken = Math.max(...[0, 1, 2].map((i) => Math.abs(refBg[i] - bg[i])));
        const lechChu = Math.max(...[0, 1, 2].map((i) => Math.abs(refFg[i] - fg[i])));
        const edge = ui.miniEdge || 'right';
        const ngang = edge === 'top' || edge === 'bottom';
        // Liệt kê mọi thứ THẬT SỰ có vẽ (nền đục/mờ hoặc có viền) trong cửa sổ thu gọn: phần dư ra
        // ngoài thanh mép từng hiện lên như khung nút bự cũ mà không rõ thủ phạm.
        const painted = [...document.querySelectorAll('*')].map((el) => {
          const r = el.getBoundingClientRect(), c2 = getComputedStyle(el);
          const nen = c2.backgroundColor;
          const vien = ['borderTopWidth', 'borderLeftWidth', 'borderRightWidth', 'borderBottomWidth']
            .reduce((s, k) => s + (parseFloat(c2[k]) || 0), 0);
          if (!(r.width && r.height) || (nen === 'rgba(0, 0, 0, 0)' && !vien)) return null;
          return (el.className || el.tagName) + ' ' + [r.left, r.top, r.width, r.height].map(Math.round).join(',') + ' ' + nen;
        }).filter(Boolean);
        return { has: true, collapsed: !!ui.miniCollapsed, arrow: el.textContent, edge,
          x: Math.round(r.left), y: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height),
          day: ngang ? Math.round(r.height) : Math.round(r.width),   // bề dày thanh theo đúng tabSize
          bg: cs.backgroundColor, theme: document.documentElement.dataset.theme,
          lechToken, lechChu, tuongPhan: Math.abs(lum(bg) - lum(fg)),
          bodyCls: document.body.className, painted,
          vw: innerWidth, vh: innerHeight, tabSize: db.settings.tabSize };
      })()`);
      console.log('TAB-VISIBLE ' + JSON.stringify(tabv) + ' win=' + JSON.stringify(W.mini().getBounds()));
      // Chụp CHÍNH cửa sổ mini ở trạng thái thu gọn rồi đếm pixel ĐÚNG MÀU người dùng đặt: cách này
      // không phụ thuộc chuyện chụp màn hình có bắt được cửa sổ trong suốt hay không, và chứng minh
      // luôn là màu custom đã được áp (không phải màu mặc định của token).
      // capturePage() có thể trả KHUNG CŨ nếu cửa sổ chưa vẽ lại (đã từng làm 4 ảnh chế độ xem giống
      // hệt nhau) -> xin 2 khung hình rồi invalidate trước khi chụp, và race với timeout vì rAF không
      // cháy khi cửa sổ bị che.
      try {
        await miniWin.webContents.executeJavaScript(`Promise.race([
          new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(() => setTimeout(r, 30)))),
          new Promise((r) => setTimeout(r, 400)),
        ])`);
        miniWin.webContents.invalidate();
      } catch { /* cửa sổ đã hủy */ }
      const tabImg = await miniWin.webContents.capturePage();
      const tabSize2 = tabImg.getSize();
      const bmp2 = tabImg.toBitmap();
      const m3 = (tabv.bg || 'rgb(0,0,0)').match(/\d+/g) || ['0', '0', '0'];
      const want = [+m3[0], +m3[1], +m3[2]];
      let khop = 0, minX = 1e9, maxX = -1, minY = 1e9, maxY = -1;
      const W2 = tabSize2.width;
      for (let i = 0, px = 0; i + 2 < bmp2.length; i += 4, px++) {
        const b = bmp2[i], g = bmp2[i + 1], r = bmp2[i + 2];
        if (Math.abs(r - want[0]) < 26 && Math.abs(g - want[1]) < 26 && Math.abs(b - want[2]) < 26) {
          khop++;
          const x = px % W2, y = (px / W2) | 0;
          if (x < minX) minX = x; if (x > maxX) maxX = x;
          if (y < minY) minY = y; if (y > maxY) maxY = y;
        }
      }
      fs.writeFileSync(path.join(__dirname, '..', 'smoke-tab.png'), tabImg.toPNG());
      const tot = tabSize2.width * tabSize2.height;
      const bbox = khop ? `${minX}..${maxX} x ${minY}..${maxY}` : 'không có';
      console.log(`TAB-PIXELS ${tabSize2.width}x${tabSize2.height} dungMau=${khop}/${tot} (${Math.round((khop / tot) * 100)}%) bbox=${bbox} (anh ${tabSize2.width}x${tabSize2.height})`);
      if (khop < tot * 0.08) errs.push(`probe: thanh mép gần như không vẽ gì (chỉ ${khop}/${tot} pixel đúng màu)`);
      W.showMini();                       // trả lại mini đang MỞ: để nguyên dạng thanh mép thì các probe sau không có danh sách mà bấm
      await waitFor(miniWin, `!!document.querySelector('.mini-list')`, 3000);
      if (!tabv.has) errs.push('probe: thu gọn mà KHÔNG vẽ thanh mép (' + JSON.stringify(tabv) + ')');
      else {
        // bề dày phải đúng tabSize người dùng đặt (không phải bị ép thành 42) và neo ở phía mép màn hình
        if (tabv.day < 4 || tabv.day > 26) errs.push('probe: bề dày thanh mép không theo tabSize (' + JSON.stringify(tabv) + ')');
        if (tabv.tuongPhan < 120) errs.push('probe: thanh mép không tương phản, coi như tàng hình (' + JSON.stringify(tabv) + ')');
        // màu phải theo hệ thống màu của app (nền tối -> xám đậm/đen, nền sáng -> xám nhạt/trắng), không theo accent
        if (tabv.lechToken > 4 || tabv.lechChu > 4) errs.push('probe: màu thanh mép không theo hệ thống màu của app (' + JSON.stringify(tabv) + ')');
        // Thu gọn thì trong cửa sổ CHỈ được có thanh mép: mọi khối khác vẽ ra đây đều là rác của
        // thiết kế cũ (từng là viền 1px của .mini dính vào body, thành khung nút bự 42x44).
        const rac = (tabv.painted || []).filter((p) => !p.startsWith('edge-tab'));
        if (rac.length) errs.push('probe: cửa sổ thu gọn còn khối lạ ngoài thanh mép (' + JSON.stringify(rac) + ')');
        const dungPhia = tabv.edge === 'right' ? Math.abs(tabv.x + tabv.w - tabv.vw) <= 2
          : tabv.edge === 'left' ? tabv.x <= 2
          : tabv.edge === 'top' ? tabv.y <= 2
          : Math.abs(tabv.y + tabv.h - tabv.vh) <= 2;
        if (!dungPhia) errs.push('probe: thanh mép không neo ở phía mép màn hình (' + JSON.stringify(tabv) + ')');
        if (tabv.w > tabv.vw + 1 || tabv.h > tabv.vh + 1) errs.push('probe: thanh mép to hơn cả cửa sổ (' + JSON.stringify(tabv) + ')');
      }
      ...
      const miniDet = await miniWin.webContents.executeJavaScript(`(async () => {
        // Đừng phụ thuộc view còn sót từ probe trước (có thể đang là thùng rác rỗng -> không có dòng nào
        // để bấm). Tự dựng trạng thái như các probe khác.
        ui.view = 'myday'; ui.openId = null; ui.miniPick = false; clearF(); render();
      ...
      if (!miniDet.why && (!miniDet.open || !miniDet.back || !miniDet.listed)) errs.push('probe: mini thiếu nút BACK về danh sách (' + JSON.stringify(miniDet) + ')');
      if (miniDet.why) errs.push('probe: không có dòng nào trong mini để kiểm nút BACK (' + JSON.stringify(miniDet) + ')');
      ...
      // Đưa mini về màn DANH SÁCH trước (probe trước có thể để nó ở màn chi tiết -> không có ô thêm việc,
      // và khi đó "không thấy việc" là do probe tự gây ra chứ không phải đồng bộ hỏng).
      await miniWin.webContents.executeJavaScript(`ui.openId = null; ui.miniPick = false; ui.view = 'myday'; ui.fold = {}; render()`);
      await waitFor(miniWin, `!!document.querySelector('.mini-add input')`);
      // Gõ + Enter có thể rơi vào lúc mini vừa vẽ lại -> thử lại vài lượt trước khi kết luận hỏng.
      const syncExpr = `db.tasks.some((t) => t.title.includes(${JSON.stringify(mark)}))`;
      let syncMain = false;
      let diag = null;
      for (let attempt = 1; attempt <= 3 && !syncMain; attempt++) {
        const typed = await miniWin.webContents.executeJavaScript(`(() => {
          const i = document.querySelector('.mini-add input');
          window.__diag = { coO: !!i, view: ui.view, openId: ui.openId, collapsed: !!ui.miniCollapsed, apiAdd: typeof API.addTask };
          if (!i) return false;
          i.value = ${JSON.stringify(mark)};
          i.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
          return true;
        })()`);
        diag = await miniWin.webContents.executeJavaScript('window.__diag || null');
        if (!typed) { await new Promise((r) => setTimeout(r, 300)); continue; }
        syncMain = await waitFor(mainWin, syncExpr, 1500);
        if (!syncMain) await new Promise((r) => setTimeout(r, 300));
      }
      const syncId = syncMain ? await mainWin.webContents.executeJavaScript(`(db.tasks.find((t) => t.title.includes(${JSON.stringify(mark)})) || {}).id`) : null;
      const miniRename = mark + '-B';
      // syncId có thể rỗng nếu việc chưa kịp sang main -> ĐỪNG để nó nổ "Cannot set properties of
      // undefined" (làm hỏng cả lượt chạy); báo đúng probe nào hỏng rồi bỏ qua phần còn lại.
      if (!syncId) {
        console.log(`SYNC mini->main=${syncMain} main->mini=SKIPPED (không thấy việc vừa thêm để đổi tên) diag=${JSON.stringify(diag)}`);
        errs.push('probe: thêm việc ở mini mà app chính không cập nhật (đồng bộ 1 chiều hỏng) (' + JSON.stringify(diag) + ')');
      } else {
      ...
      // Chờ mini vẽ lại thay vì ngủ cứng 900ms: mini đang xem danh sách nào cũng phải thấy việc đã đổi
      // tên (tên cũ biến mất khỏi mọi khung), nên chờ là điều kiện thật chứ không phải đoán thời gian.
      const miniHas = `[...document.querySelectorAll('.mini-list .row')].some((x) => x.dataset.id === ${JSON.stringify(syncId)}) || [...document.querySelectorAll('.mini .detail, .mini-list .row span')].some((e) => (e.textContent || '').includes(${JSON.stringify(miniRename)}))`;
      const syncMini = syncId ? await waitFor(miniWin, miniHas, 3000) : false;
      ...
      // LẶP THEO THỨ / NGÀY TRONG THÁNG đi qua UI THẬT: chip thứ + ô ngày + giờ, rồi tick để xem lần kế tiếp.
      // (đơn vị nextDue đã có test riêng trong lib/tasks.test.js; ở đây kiểm phần nối UI vào dữ liệu)
      const rep = await mainWin.webContents.executeJavaScript(`(async () => {
        const wait = (ms) => new Promise((r2) => setTimeout(r2, ms));
        const base = db.tasks.find((x) => !x.parentId);
        const mk = (rep2) => { const t2 = { ...base, id: uid(), title: 'REP ' + (rep2.type || 'none'), parentId: null, done: false, doneAt: null, stage: 'todo', myDay: ymd(new Date()), due: at(new Date(), 0, 7), repeat: rep2, notifiedFor: null }; db.tasks.push(t2); return t2; };
        const wk = mk({ type: 'weekly', n: 1, days: [1, 3], time: '07:00' });
        const mo = mk({ type: 'monthly', n: 1, days: [15, -1] });
        commit(); ui.view = 'myday'; ui.openId = wk.id; render(); await wait(300);
        const chips = [...document.querySelectorAll('.detail .rechip')];
        const chipNames = chips.map((c) => c.textContent);
        const onBefore = chips.filter((c) => c.classList.contains('on')).map((c) => c.textContent);
        chips[3].click(); await wait(200);                       // T5 (hàng chip là T2 T3 T4 T5 T6 T7 CN)
        const days2 = (db.tasks.find((x) => x.id === wk.id).repeat.days || []).slice();
        const onAfter = [...document.querySelectorAll('.detail .rechip')].filter((c) => c.classList.contains('on')).map((c) => c.textContent);
        const timeInput = document.querySelector('.detail input[type="time"]');
        const chipRow = document.querySelector('.body .row[data-id="' + wk.id + '"] .row-meta').textContent;
        const moChip = document.querySelector('.body .row[data-id="' + mo.id + '"] .row-meta').textContent;
        // tick việc lặp T2+T4+T5 lúc 7h -> lần kế tiếp phải ở đúng 7h và KHÔNG phải hôm nay
        ui.openId = null; render(); await wait(250);
        document.querySelector('.body .row[data-id="' + wk.id + '"] .cb').click(); await wait(450);
        const clone = db.tasks.find((x) => x.id !== wk.id && /^REP weekly/.test(x.title) && !x.done);
        const cd = clone ? new Date(clone.due) : null;
        const out = { chipNames, onBefore, days2, onAfter, hasTime: !!timeInput, chipRow, moChip,
          nextDays: clone ? (clone.repeat.days || []).slice() : null,
          nextHour: cd ? cd.getHours() : null, nextIsToday: cd ? cd.toDateString() === new Date().toDateString() : null };
        db.tasks = db.tasks.filter((x) => !/^REP /.test(x.title));
        commit(); ui.view = 'myday'; render(); await wait(150);
        return out;
      })()`);
      console.log('REPEAT-DAYS ' + JSON.stringify(rep));
      if (rep.chipNames.join(' ') !== 'T2 T3 T4 T5 T6 T7 CN') errs.push('probe: hàng chip thứ trong tuần sai (' + JSON.stringify(rep.chipNames) + ')');
      if (rep.onBefore.join(' ') !== 'T2 T4') errs.push('probe: chip thứ không hiện đúng thứ đã chọn (' + JSON.stringify(rep.onBefore) + ')');
      if (rep.days2.join(',') !== '1,3,4') errs.push('probe: bấm chip thứ không cập nhật dữ liệu lặp (' + JSON.stringify(rep.days2) + ')');
      if (rep.onAfter.join(' ') !== 'T2 T4 T5') errs.push('probe: chip thứ không đổi trạng thái sáng (' + JSON.stringify(rep.onAfter) + ')');
      if (!rep.hasTime) errs.push('probe: thiếu ô giờ của lần lặp');
      if (!/T2 T4 T5/.test(rep.chipRow) || !/07:00/.test(rep.chipRow)) errs.push('probe: chip ở danh sách không ghi rõ thứ/giờ lặp (' + rep.chipRow + ')');
      if (!/15, cuối/.test(rep.moChip)) errs.push('probe: chip lặp theo tháng không ghi rõ ngày (phải là "15, cuối") (' + rep.moChip + ')');
      if (rep.nextIsToday || rep.nextHour !== 7) errs.push('probe: lần lặp kế tiếp sai (phải ở tương lai, đúng 7h) (' + JSON.stringify(rep) + ')');
      // Âm thanh khi xong việc: bật/tắt được, có file riêng, và phát không được phép làm hỏng việc tick
      const snd = await mainWin.webContents.executeJavaScript(`(() => {
        const before = { on: db.settings.soundDone, file: db.settings.soundDoneFile, pick: typeof API.pickSound };
        let threw = null;
        try { playDone(); } catch (e) { threw = String(e && e.message); }
        db.settings.soundDone = false;
        try { playDone(); } catch (e2) { threw = String(e2 && e2.message); }
        db.settings.soundDone = before.on; commit();
        return { ...before, threw };
      })()`);
      console.log('SOUND ' + JSON.stringify(snd));
      if (snd.pick !== 'function') errs.push('probe: chưa nối được hộp chọn file âm thanh (API.pickSound: ' + snd.pick + ')');
      if (snd.threw) errs.push('probe: phát tiếng báo xong bị lỗi -> có thể làm hỏng thao tác tick (' + snd.threw + ')');
      if (snd.on === undefined) errs.push('probe: thiếu cài đặt tiếng báo khi xong việc');
      ...
      // "Hôm nay của tôi" chia 4 nhóm + "Trễ hẹn" gom theo ngày và nói rõ TRỄ BAO LÂU.
      // Probe này cũng là chốt cho whitelist preload.js: thiếu myDaySection/MYDAY_SECTIONS thì render chết
      // (TypeError) và cả app không vẽ gì, chứ không chỉ sai nhóm.
      const sec = await mainWin.webContents.executeJavaScript(`(async () => {
        const wait = (ms) => new Promise((r2) => setTimeout(r2, ms));
        const mk = (title, due, myDay) => ({ id: uid(), title, done: false, doneAt: null, stage: 'todo', createdAt: new Date().toISOString(), due, start: null, duration: 0, priority: 0, tags: [], listId: 'inbox', parentId: null, notes: '', repeat: null, remind: null, myDay, notifiedFor: null });
        db.tasks.push(
          mk('SEC hom nay', at(new Date(), 0, 15), null),
          mk('SEC tre 2 ngay', at(new Date(), -2, 9), null),
          mk('SEC khong han', null, ymd(new Date())),
          mk('SEC tu chon sau', at(new Date(), 4, 9), ymd(new Date())));
        commit(); ui.view = 'myday'; ui.mode = 'list'; ui.fold = {}; render(); await wait(350);
        const groups = [...document.querySelectorAll('.body .group')];
        const myday = groups.map((g) => {
          const rows = [...g.querySelectorAll('.row .row-title')].map((x) => x.textContent);
          return { label: g.querySelector('summary span').textContent, open: g.open, rows };
        });
        ui.view = 'late'; render(); await wait(300);
        const late = [...document.querySelectorAll('.body .group')].map((g) => ({
          label: g.querySelector('summary span').textContent,
          rows: [...g.querySelectorAll('.row .row-title')].map((x) => x.textContent),
        }));
        db.tasks = db.tasks.filter((x) => !/^SEC /.test(x.title));
        commit(); ui.view = 'myday'; render(); await wait(150);
        return { myday, late };
      })()`);
      console.log('MYDAY-SECTIONS ' + JSON.stringify(sec));
      const nhan = sec.myday.map((g) => g.label);
      if (nhan.length !== 4) errs.push('probe: "Hôm nay của tôi" không chia đủ 4 nhóm (' + JSON.stringify(sec.myday) + ')');
      if (!nhan[0].startsWith('◉') || !nhan[1].includes('Trễ') || !nhan[2].includes('Chưa có hạn') || !nhan[3].includes('Tự chọn')) {
        errs.push('probe: nhãn/sai thứ tự nhóm trong "Hôm nay của tôi" (' + JSON.stringify(nhan) + ')');
      }
      // mỗi việc phải nằm ĐÚNG 1 nhóm, đúng nhóm theo lý do của nó (nhóm còn việc khác của fixture nên chỉ so "có mặt")
      const wantSec = ['SEC hom nay', 'SEC tre 2 ngay', 'SEC khong han', 'SEC tu chon sau'];
      wantSec.forEach((nm, i) => {
        const got = (sec.myday[i] || {}).rows || [];
        if (!got.some((x) => x.includes(nm))) errs.push('probe: "' + nm + '" không nằm đúng nhóm trong "Hôm nay của tôi" (' + JSON.stringify(sec.myday) + ')');
      });
      const allRows = sec.myday.flatMap((g) => g.rows);
      for (const nm of wantSec) {
        if (allRows.filter((x) => x.includes(nm)).length !== 1) errs.push('probe: "' + nm + '" nằm ở 0 nhóm hoặc bị lặp ở nhiều nhóm (' + JSON.stringify(allRows) + ')');
      }
      if (!sec.myday.every((g) => g.open)) errs.push('probe: nhóm trong "Hôm nay của tôi" không mở sẵn');
      // "Trễ hẹn": việc trễ 2 ngày phải nằm trong thư mục của NGÀY đến hạn, nhãn ghi rõ trễ bao lâu
      if (!sec.late.some((g) => /Trễ 2 ngày/.test(g.label) && g.rows.some((x) => x.includes('SEC tre 2 ngay')))) {
        errs.push('probe: "Trễ hẹn" không gom theo ngày / không ghi rõ trễ bao lâu (' + JSON.stringify(sec.late) + ')');
      }
      // "Hôm nay" phải có việc LẶP rơi vào hôm nay — việc lặp thường KHÔNG có `due`, nên chỉ soi `due`
      // là đủ để nó hiện ở "Hôm nay của tôi" mà KHÔNG BAO GIỜ hiện ở "Hôm nay" (lỗi user báo).
      // Kèm 2 việc nhỏ: sidebar ẩn số ở "Đã hoàn thành"/"Đã xoá", và đổi tên được danh sách.
      const plan = await mainWin.webContents.executeJavaScript(`(async () => {
        const wait = (ms) => new Promise((r2) => setTimeout(r2, ms));
        const now = new Date(), wd = now.getDay(), last = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
        const mk = (title, repeat) => ({ id: uid(), title, done: false, doneAt: null, stage: 'todo', createdAt: new Date().toISOString(), due: null, start: null, duration: 0, priority: 0, tags: [], listId: 'inbox', parentId: null, notes: '', repeat, remind: null, myDay: null, notifiedFor: null });
        const mai = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 9, 0);
        db.tasks.push(
          mk('SEC lap hom nay', { type: 'weekly', n: 1, days: [wd] }),
          mk('SEC lap mai', { type: 'weekly', n: 1, days: [(wd + 1) % 7] }),
          mk('SEC lap hang thang', { type: 'monthly', n: 1, days: [now.getDate() === last ? -1 : now.getDate()] }),
          // KỲ LẶP ĐÃ CÓ HẠN RIÊNG (kỳ kế tiếp sinh ra lúc tick luôn kèm hạn): HẠN quyết định ngày,
          // không được hiện sớm ở "Hôm nay" chỉ vì nó là việc hằng ngày (lỗi user báo).
          { ...mk('SEC lap co han mai', { type: 'daily', n: 1 }), due: mai.toISOString() });
        commit(); ui.view = 'today'; ui.mode = 'list'; ui.fold = {}; render(); await wait(350);
        const today = [...document.querySelectorAll('.body .row .row-title')].map((x) => x.textContent);
        ui.view = 'myday'; render(); await wait(300);
        const myday = [...document.querySelectorAll('.body .group')].map((g) => ({
          label: g.querySelector('summary span').textContent,
          rows: [...g.querySelectorAll('.row .row-title')].map((x) => x.textContent) }));
        ui.view = 'today'; render(); await wait(250);
        const navs = [...document.querySelectorAll('.sidebar .nav')].map((n) => ({
          name: [...n.children].filter((c) => c.tagName === 'SPAN' && !c.className).map((c) => c.textContent).join(''),
          ct: n.querySelector('.ct') ? n.querySelector('.ct').textContent : null }));
        // đổi tên danh sách ngay tại sidebar: bấm ✎ -> sửa trong ô -> Enter
        const btn = [...document.querySelectorAll('.sidebar .nav span[title="Đổi tên danh sách"]')][0];
        const truoc = db.lists[0] && db.lists[0].name;
        let coO = false, sau = null, hien = false;
        if (btn) {
          btn.click(); await wait(250);
          const inp = document.querySelector('.sidebar input');
          coO = !!inp;
          if (inp) { inp.value = 'SEC ten moi'; inp.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter' })); await wait(300); }
          sau = db.lists[0] && db.lists[0].name;
          hien = [...document.querySelectorAll('.sidebar .nav')].some((n) => n.textContent.includes('SEC ten moi'));
        }
        // "Định kì" = kho quản lý việc lặp: mỗi việc 1 dòng, KHÔNG có nút tick, gom nhóm theo lịch
        ui.view = 'recur'; ui.fold = {}; render(); await wait(300);
        const rec = {
          rows: [...document.querySelectorAll('.body .row .row-title')].map((x) => x.textContent),
          groups: [...document.querySelectorAll('.body .group')].map((g) => g.querySelector('summary span').textContent),
          nutTick: document.querySelectorAll('.body .row .cb:not(.recur-slot)').length,
          oTrong: document.querySelectorAll('.body .row .cb.recur-slot').length,
        };
        ui.view = 'nodate'; render(); await wait(250);
        const nodate = [...document.querySelectorAll('.body .row .row-title')].map((x) => x.textContent);
        if (db.lists[0] && truoc) db.lists[0].name = truoc;
        db.tasks = db.tasks.filter((x) => !/^SEC /.test(x.title));
        commit(); ui.view = 'myday'; render(); await wait(200);
        return { today, myday, navs, coO, truoc, sau, hien, rec, nodate };
      })()`);
      console.log('PLAN-TODAY ' + JSON.stringify(plan));
      if (!plan.today.some((x) => x.includes('SEC lap hom nay'))) errs.push('probe: việc LẶP rơi vào hôm nay không hiện ở "Hôm nay" (' + JSON.stringify(plan.today) + ')');
      if (!plan.today.some((x) => x.includes('SEC lap hang thang'))) errs.push('probe: việc lặp theo NGÀY TRONG THÁNG rơi vào hôm nay không hiện ở "Hôm nay" (' + JSON.stringify(plan.today) + ')');
      if (plan.today.some((x) => x.includes('SEC lap mai'))) errs.push('probe: "Hôm nay" lôi cả việc lặp của ngày MAI vào (' + JSON.stringify(plan.today) + ')');
      // kỳ lặp ĐÃ CÓ HẠN riêng của chính nó (16/9, 17/9…) -> hạn quyết định, không hiện sớm hôm nay
      if (plan.today.some((x) => x.includes('SEC lap co han mai'))) errs.push('probe: kỳ lặp có HẠN MAI vẫn hiện ở "Hôm nay" (' + JSON.stringify(plan.today) + ')');
      if (plan.myday.flatMap((g) => g.rows).some((x) => x.includes('SEC lap co han mai'))) errs.push('probe: kỳ lặp có hạn mai lọt vào "Hôm nay của tôi" (' + JSON.stringify(plan.myday) + ')');
      // 2 khung phải NHẤT QUÁN: việc lặp hôm nay cũng nằm trong "Hôm nay của tôi", nhóm "Chưa có hạn"
      const nod = (plan.myday.find((g) => g.label.includes('Chưa có hạn')) || {}).rows || [];
      for (const nm of ['SEC lap hom nay', 'SEC lap hang thang']) {
        if (!nod.some((x) => x.includes(nm))) errs.push('probe: "' + nm + '" không nằm trong "Hôm nay của tôi" (' + JSON.stringify(plan.myday) + ')');
      }
      // sidebar: "Đã hoàn thành" / "Đã xoá" KHÔNG hiện số, các mục khác vẫn hiện
      for (const nm of ['Đã hoàn thành', 'Đã xoá']) {
        const n = plan.navs.find((x) => x.name === nm);
        if (!n) errs.push('probe: không thấy mục "' + nm + '" ở sidebar (' + JSON.stringify(plan.navs.map((x) => x.name)) + ')');
        else if (n.ct) errs.push('probe: mục "' + nm + '" vẫn còn hiện số (' + n.ct + ')');
      }
      const hd = plan.navs.find((x) => x.name === 'Hôm nay');
      if (!hd || !hd.ct) errs.push('probe: mục "Hôm nay" mất số đếm (' + JSON.stringify(plan.navs) + ')');
      // "Định kì" = kho quản lý việc lặp: có đủ các việc lặp, KHÔNG có nút tick ở khung này, và việc
      // lặp KHÔNG còn nằm lẫn trong "Chưa có hạn".
      for (const nm of ['SEC lap hom nay', 'SEC lap hang thang', 'SEC lap co han mai']) {
        if (!plan.rec.rows.some((x) => x.includes(nm))) errs.push('probe: "' + nm + '" không nằm trong khung "Định kì" (' + JSON.stringify(plan.rec.rows) + ')');
      }
      if (plan.rec.nutTick) errs.push('probe: khung "Định kì" vẫn còn ' + plan.rec.nutTick + ' nút tick (phải là kho quản lý, không tick tại đây)');
      if (!plan.rec.oTrong) errs.push('probe: khung "Định kì" thiếu ô giữ chỗ cho thẳng hàng');
      if (!plan.rec.groups.length || !plan.rec.groups.every((g) => g.startsWith('↻ '))) errs.push('probe: khung "Định kì" không gom nhóm theo lịch (' + JSON.stringify(plan.rec.groups) + ')');
      if (plan.nodate.some((x) => /SEC lap/.test(x))) errs.push('probe: việc LẶP vẫn nằm trong "Chưa có hạn" (' + JSON.stringify(plan.nodate) + ')');

      // đổi tên danh sách
      if (!plan.coO) errs.push('probe: bấm ✎ không mở được ô đổi tên danh sách');
      else if (plan.sau !== 'SEC ten moi') errs.push('probe: đổi tên danh sách không lưu (' + plan.truoc + ' -> ' + plan.sau + ')');
      if (plan.coO && !plan.hien) errs.push('probe: sidebar không hiện tên danh sách mới');
      ...
      // "Hôm nay của tôi" = việc trong KẾ HOẠCH HÔM NAY (đến hạn hôm nay / lặp rơi vào hôm nay) ∪ việc
      // đã TRỄ ∪ việc được gắn cờ ☀. LUẬT người dùng chốt: MỌI việc ở khung "Hôm nay" đều phải nằm
      // trong "Hôm nay của tôi" -> việc đến hạn/lặp hôm nay KHÔNG bỏ ra được; việc chưa có hạn thì mặc
      // định ở ngoài, bấm ☀ mới vào (và bấm lần nữa thì ra).
      const mdr = await mainWin.webContents.executeJavaScript(`(async () => {
        ...
        const t = db.tasks.find((x) => !x.done && !x.parentId);
        if (!t) return { why: 'không có việc nào' };
        const live = () => db.tasks.find((x) => x.id === t.id);
        const due0 = t.due, md0 = t.myDay, skip0 = t.myDaySkip, rep0 = t.repeat;
        const cnt = () => document.querySelectorAll('.body .row[data-id="' + t.id + '"]').length;
        const btn = () => document.querySelector('.body .row[data-id="' + t.id + '"] .myday-btn');
        // (1) việc có hạn HÔM NAY, không gắn cờ -> vẫn tự nằm trong "Hôm nay của tôi"
        t.due = at(new Date(), 0, 23); t.myDay = null; t.myDaySkip = null; t.repeat = null; commit(); render(); await wait(300);
        const nAuto = cnt();
        // (2) bấm nút My Day trên việc đang đến hạn -> KHÔNG bỏ ra được (đúng luật), chỉ hiện toast lý do
        if (btn()) btn().click();
        await wait(350);
        const nDenHan = cnt(), toastDenHan = !!document.querySelector('.toast');
        // (3) việc LẶP rơi vào hôm nay -> cũng tự nằm trong đó
        t.due = null; t.myDay = null; t.myDaySkip = null; t.repeat = { type: 'weekly', n: 1, days: [new Date().getDay()] };
        commit(); render(); await wait(300);
        const nLap = cnt();
        // (4) việc chưa có hạn & không lặp -> mặc định KHÔNG nằm trong My Day; bấm ☀ (phải bấm từ khung
        // khác — việc chưa có hạn không có dòng nào trong My Day để mà bấm) mới vào, bấm nữa thì ra
        t.due = null; t.myDay = null; t.myDaySkip = null; t.repeat = null; commit(); ui.view = 'myday'; render(); await wait(300);
        const nPlain = cnt();
        ui.view = 'nodate'; render(); await wait(300);
        const coDongONodate = !!btn();
        ...
        ui.view = 'myday'; render(); await wait(300);
        const nUnpicked = cnt();
        // (5) LUẬT TỔNG: mọi việc ở view "Hôm nay" đều phải nằm trong "Hôm nay của tôi"
        const now = new Date();
        const thieu = T.view(db.tasks, 'today', now).filter((x) => !T.inMyDay(x, now)).map((x) => x.title);
        const liveT = live();
        liveT.due = due0; liveT.myDay = md0; liveT.myDaySkip = skip0; liveT.repeat = rep0;
        commit(); ui.view = 'myday'; render(); await wait(200);
        return { nAuto, nDenHan, toastDenHan, nLap, nPlain, coDongONodate, nPicked, nUnpicked, thieu };
      })()`);
      console.log('MYDAY-AUTO ' + JSON.stringify(mdr));
      if (mdr.nAuto !== 1) errs.push('probe: việc đến hạn hôm nay không tự nằm trong "Hôm nay của tôi" (' + JSON.stringify(mdr) + ')');
      if (mdr.nDenHan !== 1) errs.push('probe: việc ĐẾN HẠN hôm nay bị bỏ ra khỏi "Hôm nay của tôi" (luật: mọi việc ở "Hôm nay" đều nằm trong đó) (' + JSON.stringify(mdr) + ')');
      if (!mdr.toastDenHan) errs.push('probe: bấm nút My Day mà không báo gì cho người dùng biết vì sao (' + JSON.stringify(mdr) + ')');
      if (mdr.nLap !== 1) errs.push('probe: việc LẶP rơi vào hôm nay không tự nằm trong "Hôm nay của tôi" (' + JSON.stringify(mdr) + ')');
      if (mdr.nPlain !== 0) errs.push('probe: việc chưa có hạn lại nằm sẵn trong "Hôm nay của tôi" (phải bấm ☀ mới vào) (' + JSON.stringify(mdr) + ')');
      if (!mdr.coDongONodate) errs.push('probe: không thấy dòng việc ở khung "Chưa có hạn" (' + JSON.stringify(mdr) + ')');
      if (mdr.nPicked !== 1) errs.push('probe: bấm ☀ không thêm được việc chưa có hạn vào "Hôm nay của tôi" (' + JSON.stringify(mdr) + ')');
      if (mdr.nUnpicked !== 0) errs.push('probe: bấm ☀ lần nữa không bỏ được việc ra khỏi "Hôm nay của tôi" (' + JSON.stringify(mdr) + ')');
      if (mdr.thieu && mdr.thieu.length) errs.push('probe: có việc nằm ở "Hôm nay" mà KHÔNG nằm trong "Hôm nay của tôi" (' + JSON.stringify(mdr.thieu) + ')');

      // "TRỄ HẸN": gom mọi việc quá hạn; việc quá hạn phải RỜI "Hôm nay" nhưng VẪN nằm trong "Hôm nay của tôi"
      const latev = await mainWin.webContents.executeJavaScript(`(async () => {
        const wait = (ms) => new Promise((r2) => setTimeout(r2, ms));
        const t = db.tasks.find((x) => !x.done && !x.parentId);
        if (!t) return { why: 'không có việc nào' };
        const due0 = t.due, md0 = t.myDay, skip0 = t.myDaySkip;
        const past = new Date(Date.now() - 5 * 864e5);
        t.due = past.toISOString(); t.myDay = null; t.myDaySkip = null; commit(); await wait(150);
        const now = new Date();
        const inLate = T.view(db.tasks, 'late', now).some((x) => x.id === t.id);
        const inToday = T.view(db.tasks, 'today', now).some((x) => x.id === t.id);
        const inMyDay = T.view(db.tasks, 'myday', now).some((x) => x.id === t.id);
        ui.view = 'late'; ui.fold = {}; ui.openId = null; clearF(); render(); await wait(350);
        const rows = document.querySelectorAll('.body .row[data-id="' + t.id + '"]').length;
        const title = ((document.querySelector('.title') || {}).textContent || '').replace(/\\s+/g, ' ').trim();
        const navOn = (document.querySelector('.sidebar .nav.on') || {}).textContent || '';
        ui.view = 'myday'; render(); await wait(150);
        t.due = due0; t.myDay = md0; t.myDaySkip = skip0; commit(); render(); await wait(150);
        return { inLate, inToday, inMyDay, rows, title: title.slice(0, 30), navOn: navOn.replace(/\\s+/g, ' ').trim() };
      })()`);
      console.log('LATE-VIEW ' + JSON.stringify(latev));
      if (latev.why) errs.push('probe: không có việc nào để kiểm "Trễ hẹn" (' + JSON.stringify(latev) + ')');
      else {
        if (!latev.inLate) errs.push('probe: việc quá hạn không nằm trong "Trễ hẹn" (' + JSON.stringify(latev) + ')');
        if (latev.inToday) errs.push('probe: việc quá hạn vẫn còn trong "Hôm nay" (phải chuyển sang "Trễ hẹn") (' + JSON.stringify(latev) + ')');
        if (!latev.inMyDay) errs.push('probe: "Hôm nay của tôi" bị mất việc quá hạn (' + JSON.stringify(latev) + ')');
        if (latev.rows !== 1) errs.push('probe: chọn "Trễ hẹn" mà danh sách không hiện việc quá hạn (' + JSON.stringify(latev) + ')');
        if (!latev.title.includes('Trễ hẹn')) errs.push('probe: tiêu đề view "Trễ hẹn" sai (' + JSON.stringify(latev) + ')');
      }
      // THÙNG RÁC: việc đã xoá quá 30 ngày phải bị dọn hẳn, việc mới xoá thì giữ
      {
        const P = require('./store');
        const probeDb = {
          trash: [
            { id: 'x-cu', title: 'xoá lâu rồi', deletedAt: new Date(Date.now() - 40 * 864e5).toISOString() },
            { id: 'x-moi', title: 'vừa xoá', deletedAt: new Date().toISOString() },
            { id: 'x-khong-ngay', title: 'thiếu deletedAt' },
          ],
        };
        const removed = P.purgeTrash(probeDb, 30);
        const left = probeDb.trash.map((t) => t.id);
        console.log(`TRASH-PURGE removed=${removed} left=${JSON.stringify(left)}`);
        if (removed !== 1 || left.join(',') !== 'x-moi,x-khong-ngay') errs.push('probe: dọn thùng rác quá 30 ngày sai (' + JSON.stringify({ removed, left }) + ')');
      }
      ...
      // Tra ĐÍCH DANH theo tên value: trước đây lọc theo 'electron.exe' nên không phân biệt được
      // entry thật của người dùng với entry của smoke -> smoke tắt là xoá luôn auto-start thật.
      const A = require('./autostart');
      const regVal = (name) => {
        try {
          return require('node:child_process').execFileSync('reg', ['query', runKey, '/v', name], { encoding: 'utf8' })
            .split('\n').filter((l) => l.includes('electron.exe')).map((l) => l.trim().replace(/[ \t]+/g, ' ')).join(' | ');
        } catch { return ''; }
      };
      const realBefore = regVal('com.thong.todoapp');
      const asOn = await mainWin.webContents.executeJavaScript(`window.api.autostart(true)`);
      const asReg = regVal(A.NAME);
      const asOff = await mainWin.webContents.executeJavaScript(`window.api.autostart(false)`);
      const asGone = regVal(A.NAME) === '';
      const realAfter = regVal('com.thong.todoapp');
      console.log(`AUTOSTART on=${asOn} writing=${asOff} ten="${A.NAME}" reg="${asReg}" cleaned=${asGone} entryThat="${realAfter}"`);
      ...
      if (realAfter !== realBefore) errs.push('probe: smoke ĐỤNG vào auto-start thật của người dùng (' + realBefore + ' -> ' + realAfter + ')');
      ...
      const mouseDrag = async (win, sel) => {
        ...
        // Lấy 2 dòng CÙNG MỘT NHÓM: kéo một dòng sang nhóm khác thì nó vẫn nằm nguyên trong nhóm của nó
        // (nhóm tính lại theo lý do), nên thứ tự hiển thị không đổi và phép so "2 dòng đầu đổi chỗ" vô nghĩa.
        const pick = `(() => {
          const all0 = [...document.querySelectorAll(${JSON.stringify(sel)})];
          const gs = [...document.querySelectorAll('.body .group')];
          const gi = gs.findIndex((x) => x.querySelectorAll('.row:not(.sub)').length >= 2);
          const all = gi >= 0 ? [...gs[gi].querySelectorAll('.row:not(.sub)')] : all0;
          if (all[1]) all[1].scrollIntoView({ block: "center" });
          return { gl: gi >= 0 ? gs[gi].querySelector('summary span').textContent : null,
            rows: all.map((r) => { const b = r.getBoundingClientRect(); return { id: r.dataset.id, x: b.x + b.width * 0.4, y: b.y + b.height / 2, h: b.height, tag: r.tagName, cls: r.className, vh: window.innerHeight }; }) };
        })()`;
        const picked = await win.webContents.executeJavaScript(pick);
        const rects = picked.rows;
        ...
        await new Promise((r) => setTimeout(r, 250));
        // so ĐÚNG trong nhóm đã chọn (nhóm tính lại theo lý do nên số dòng có thể đổi sau khi thả;
        // tìm lại nhóm theo NHÃN chứ không theo chỉ số, tránh lệch khi có nhóm biến mất)
        const afterExp = `(() => {
          const g = ${JSON.stringify(picked.gl)} === null ? null
            : [...document.querySelectorAll('.body .group')].find((x) => x.querySelector('summary span').textContent === ${JSON.stringify(picked.gl)});
          const rows = g ? [...g.querySelectorAll('.row:not(.sub)')] : [...document.querySelectorAll(${JSON.stringify(sel)})];
          return rows.map((r) => r.dataset.id);
        })()`;
        const after = await win.webContents.executeJavaScript(afterExp);
        // ngữ cảnh khi fail: thứ tự có thể không đổi vì lý do hợp lệ (2 dòng khác nhóm, bị lớp khác che,
        // sort không phải 'manual'...) nên soi thẳng ra thay vì đoán.
        const ctx = await win.webContents.executeJavaScript(`(() => {
          const h = document.elementFromPoint(${Math.round(rects[1].x)}, ${Math.round(rects[1].y)});
          return { sort: ui.sort, view: ui.view, mode: ui.mode, open: ui.openId, nhom: ${JSON.stringify(picked.gl)},
            groups: [...document.querySelectorAll('.body .row:not(.sub) .row-title')].map((x) => x.textContent).slice(0, 5),
            hit: h ? (h.className || h.tagName) : 'null', dragId };
        })()`);
        return { ok: after[0] === rects[1].id && after[1] === rects[0].id, before: rects.map((r) => r.id), after, ctx };
      };
      ...
      // capturePage() trên cửa sổ không vẽ lại trả về KHUNG CŨ (ảnh kanban/lịch/timeline/ma trận
      // từng giống hệt nhau). -> xin 2 khung hình + invalidate() trước khi chụp.
      // NHƯNG: requestAnimationFrame KHÔNG cháy khi cửa sổ bị che -> promise treo vĩnh viễn, cả lượt
      // smoke đứng im (đã bị đúng như vậy). Phải race với timeout để luôn có đường thoát.
      const shoot = async (name) => {
        const w = name.startsWith('mini') ? miniWin : mainWin;
        // Phải race ở phía MAIN, không phải race bên trong renderer: nếu renderer đang bận thì cái
        // setTimeout({400}) nằm TRONG đó cũng không bao giờ cháy, và `await executeJavaScript` treo
        // vĩnh viễn -> cả lượt smoke đứng im (watchdog chỉ báo "có treo", không nói treo ở đâu).
        try {
          await Promise.race([
            // Giữ nguyên nhịp cũ (2 khung hình + 50ms lắng) vì chính nhịp đó làm capturePage hết trả khung
            // cũ. Chỉ ĐỔI CHỖ race: timeout phải nằm phía MAIN — nếu renderer đang bận thì setTimeout
            // bên trong nó cũng không cháy, `await executeJavaScript` treo vĩnh viễn và cả lượt smoke
            // đứng im (watchdog chỉ báo "có treo", không nói treo ở đâu).
            w.webContents.executeJavaScript(`new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(() => setTimeout(r, 50))))`),
            new Promise((r) => setTimeout(() => r(false), 900)),
          ]);
          w.webContents.invalidate();
        } catch { /* cửa sổ đã hủy thì bỏ qua */ }
        // capturePage() cũng phải có timeout: nó treo vĩnh viễn khi cửa sổ bị che/mất kết nối vẽ ->
        // lượt smoke dài đứng im ở đúng dòng này (watchdog chỉ báo được là có treo, không nói treo ở đâu).
        const img = await Promise.race([
          w.webContents.capturePage(),
          new Promise((r) => setTimeout(() => r(null), 5000)),
        ]);
        if (!img || img.isEmpty()) {
          errs.push('probe: chụp ảnh "' + name + '" không xong trong 5s (bỏ qua ảnh, không phải lỗi app)');
          return '';
        }
        const png = img.toPNG();
      ...
      const md5 = (b) => require('node:crypto').createHash('md5').update(b || '').digest('hex');
      ...
      const modeShot = async (i, name, sel) => {
        ...
        // capturePage() hay trả KHUNG CŨ khi máy bận -> chụp lại vài lần rồi mới kết luận
        // "các chế độ giống nhau" (đã gặp: 2/3 lượt báo trùng trong khi ảnh thật khác nhau).
        // Lượt chụp lại phải ĐỔI CẤU TRÚC DOM (sang chế độ khác rồi quay lại), không chỉ `render()`:
        // render() vẽ lại cùng cây DOM nên compositor có thể bỏ qua, ảnh vẫn là khung cũ.
        for (let lan = 1; lan <= 4; lan++) {
          await new Promise((r) => setTimeout(r, 350 * lan));
          shots[name] = md5(await shoot(name));
          if (!Object.entries(shots).some(([n2, v]) => n2 !== name && v === shots[name])) return;
          await clickTab(i === 1 ? 2 : 1);
          await new Promise((r) => setTimeout(r, 250));
          await clickTab(i);
          if (!(await waitFor(mainWin, `!!document.querySelector(${JSON.stringify(sel)})`))) errs.push('probe: chế độ ' + name + ' không render ra ' + sel);
        }
      ...
      const card = await mainWin.webContents.executeJavaScript(`(() => {
        ui.modal = 'help'; ui.helpTab = 'guide'; render();
      ...
      const help = await mainWin.webContents.executeJavaScript(`(async () => {
        ...
        ui.helpTab = 'guide';   // hộp thoại nhớ tab -> ép tab 1 cho chắc
        ...
        const out = {
          ...
          tabs: [...m.querySelectorAll('.help-tabs button')].map((b) => b.textContent),
          ver: ((m.querySelector('.ver') || {}).textContent || '').trim(),
        };
        // TAB 2: giải thích chi tiết khung việc (từng mục trong dòng việc + nút dễ gây khó hiểu)
        const b2 = [...m.querySelectorAll('.help-tabs button')].find((x) => /khung việc/.test(x.textContent));
        if (b2) { b2.click(); await wait(320); }
        const m2 = document.querySelector('#helpModal');
        out.tab2 = m2 ? {
          on2: [...m2.querySelectorAll('.help-tabs button')].some((x) => /khung việc/.test(x.textContent) && x.classList.contains('on')),
          anatomy: m2.textContent.includes('Chấm ƯU TIÊN'),
          btns: m2.textContent.includes('Hôm nay của tôi') && m2.textContent.includes('BẢNG LỌC'),
          authorGone: !m2.querySelector('.author'),
        } : {};
        // quay lại tab 1 rồi mới thử Esc
        const b1 = [...m2.querySelectorAll('.help-tabs button')].find((x) => /Hướng dẫn/.test(x.textContent));
        if (b1) { b1.click(); await wait(320); }
        out.backFirst = !!document.querySelector('#helpModal .author');
      ...
      if (!help.tabs || help.tabs.length !== 2) errs.push('probe: hộp Hướng dẫn không có đúng 2 tab (' + JSON.stringify(help.tabs) + ')');
      if (!/^v\d+\.\d+\.\d+$/.test(help.ver || '')) errs.push('probe: hộp Hướng dẫn không hiện số phiên bản (' + help.ver + ')');
      if (help.ver !== 'v' + require('../package.json').version) errs.push('probe: phiên bản hiện trong app khác package.json (' + help.ver + ')');
      if (!help.tab2 || !help.tab2.on2) errs.push('probe: bấm tab 2 không chuyển tab (' + JSON.stringify(help.tab2) + ')');
      if (!help.tab2 || !help.tab2.anatomy || !help.tab2.btns) errs.push('probe: tab 2 thiếu phần giải thích khung việc / nút khó hiểu (' + JSON.stringify(help.tab2) + ')');
      if (!help.tab2 || !help.tab2.authorGone) errs.push('probe: tab 2 vẫn hiện nội dung của tab 1 (' + JSON.stringify(help.tab2) + ')');
      if (!help.backFirst) errs.push('probe: quay lại tab 1 không thấy lại nội dung hướng dẫn');
      if (!help.escCloses) errs.push('probe: Esc không đóng được hộp thoại Hướng dẫn');
      await mainWin.webContents.executeJavaScript(`ui.modal = 'help'; ui.helpTab = 'guide'; render()`);
      await new Promise((r) => setTimeout(r, 350));
      const bHelp = await shoot('help');
      const hHelp = md5(bHelp);
      // capturePage() hay trả KHUNG CŨ -> ảnh tab 2 y hệt ảnh tab 1 dù DOM đã đổi. Chụp lại tới khi khác.
      // Chỉ `render()` là chưa đủ (DOM giống nhau về cấu trúc -> compositor không buộc vẽ lại): đóng modal
      // rồi mở lại để đổi hẳn cây DOM, và chờ tăng dần. Kèm log kích thước byte để phân biệt được
      // "khung cũ" (2 ảnh y hệt từng byte) với "ảnh giống thật".
      let hFrame = hHelp, bFrame = bHelp, dom2 = false;
      for (let lan = 1; lan <= 4 && hFrame === hHelp; lan++) {
        await mainWin.webContents.executeJavaScript(`ui.modal = null; render()`);
        await new Promise((r) => setTimeout(r, 200));
        dom2 = await mainWin.webContents.executeJavaScript(`(() => { ui.modal = 'help'; ui.helpTab = 'frame'; render(); return !document.querySelector('#helpModal .author') && document.querySelectorAll('#helpModal .helpitem').length > 5; })()`);
        await new Promise((r) => setTimeout(r, 400 + 250 * lan));
        bFrame = await shoot('help-frame');
        hFrame = md5(bFrame);
      }
      console.log('HELP-SHOTS ' + JSON.stringify({ tab2KhacTab1: hFrame !== hHelp, dom2, bytes: [bHelp.length, bFrame.length] }));
      if (hFrame === hHelp) errs.push('probe: ảnh 2 tab Hướng dẫn y hệt nhau (tab 2 không vẽ ra hoặc capturePage trả khung cũ) ' + JSON.stringify({ dom2, bytes: [bHelp.length, bFrame.length] }));
      await mainWin.webContents.executeJavaScript(`ui.helpTab = 'guide'; render()`);
