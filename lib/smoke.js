  ...
  Promise.all([wait(mainWin, 'main'), wait(miniWin, 'mini')]).then(async () => {
    try {
      ...
      const tabv = await miniWin.webContents.executeJavaScript(`(() => {
        ...
        const lum = (c) => { const m = c.match(/\\d+/g) || []; const [R, G, B] = [+m[0], +m[1], +m[2]];
          return Math.round(0.299 * R + 0.587 * G + 0.114 * B); };
        const bgL = lum(cs.backgroundColor), fgL = lum(cs.color);
        return { has: true, collapsed: !!ui.miniCollapsed, arrow: el.textContent,
          x: Math.round(r.left), y: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height),
          bg: cs.backgroundColor, bgL, fgL, tuongPhan: Math.abs(bgL - fgL),
          vw: innerWidth, vh: innerHeight };
      })()`);
      console.log('TAB-VISIBLE ' + JSON.stringify(tabv) + ' win=' + JSON.stringify(W.mini().getBounds()));
      // Chụp CHÍNH cửa sổ mini ở trạng thái thu gọn rồi đếm pixel sáng: cách này không phụ thuộc
      // chuyện chụp màn hình có bắt được cửa sổ trong suốt hay không.
      const tabImg = await miniWin.webContents.capturePage();
      const tabSize2 = tabImg.getSize();
      const bmp2 = tabImg.toBitmap();
      let bright = 0;
      for (let i = 0; i + 2 < bmp2.length; i += 4) {
        const b = bmp2[i], g = bmp2[i + 1], r = bmp2[i + 2];
        if (0.299 * r + 0.587 * g + 0.114 * b > 150) bright++;
      }
      fs.writeFileSync(path.join(__dirname, '..', 'smoke-tab.png'), tabImg.toPNG());
      const tot = tabSize2.width * tabSize2.height;
      console.log(`TAB-PIXELS ${tabSize2.width}x${tabSize2.height} sang=${bright}/${tot} (${Math.round((bright / tot) * 100)}%)`);
      if (bright < tot * 0.3) errs.push(`probe: thanh mép gần như không vẽ gì (chỉ ${bright}/${tot} pixel sáng)`);
      ...
      if (!tabv.has) errs.push('probe: thu gọn mà KHÔNG vẽ thanh mép (' + JSON.stringify(tabv) + ')');
        ...
        if (tabv.w < 6 || tabv.h < 24) errs.push('probe: thanh mép quá nhỏ để bấm (' + JSON.stringify(tabv) + ')');
        if (tabv.tuongPhan < 120) errs.push('probe: thanh mép không tương phản, coi như tàng hình (' + JSON.stringify(tabv) + ')');
        if (tabv.x > 4 || tabv.y > 4) errs.push('probe: thanh mép không neo ở góc trên-trái cửa sổ (dễ bị đẩy ra ngoài màn hình) (' + JSON.stringify(tabv) + ')');
