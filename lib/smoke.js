...
module.exports = function smoke() {
  ...
  Promise.all([wait(mainWin, 'main'), wait(miniWin, 'mini')]).then(async () => {
    try {
      ...
      // "Hôm nay của tôi": việc ĐẶT HÔM NAY phải tự nằm trong đó (lỗi cũ: chỉ có việc tự chọn ☀ nên
      // việc đặt hôm nay không thấy đâu cả), và bấm bỏ thì phải biến mất thật (ghi myDaySkip) —
      // không được tự mọc lại ngay (lỗi cũ khác) và cũng không được mất khỏi view "Hôm nay".
      const mdr = await mainWin.webContents.executeJavaScript(`(async () => {
        ...
        // (1) việc có hạn HÔM NAY nhưng KHÔNG tự chọn -> vẫn phải hiện trong "Hôm nay của tôi"
        t.due = at(new Date(), 0, 23); t.myDay = null; t.myDaySkip = null; commit(); render(); await wait(300);
        const nAuto = cnt();
        // (2) bấm ☁ để bỏ ra -> biến mất + ghi myDaySkip = hôm nay (không tự mọc lại)
        document.querySelector('.body .row[data-id="' + t.id + '"] .myday-btn').click();
        await wait(400);
        const live = db.tasks.find((x) => x.id === t.id);
        const nGone = cnt(), skip = live.myDaySkip, md = live.myDay;
        const stillToday = T.view(db.tasks, 'today').some((x) => x.id === t.id);
        // (3) hôm sau (hết skip, việc thành trễ hạn) -> tự hiện lại
        const tomorrow = new Date(at(new Date(), 0, 23)); tomorrow.setDate(tomorrow.getDate() + 1);
        const backTomorrow = T.view(db.tasks, 'myday', tomorrow).some((x) => x.id === t.id);
        // trả nguyên trạng
        live.due = due0; live.myDay = md0; live.myDaySkip = skip0; commit(); render(); await wait(200);
        return { nAuto, nGone, md, skip, stillToday, backTomorrow };
      })()`);
      console.log('MYDAY-AUTO ' + JSON.stringify(mdr));
      if (mdr.nAuto !== 1) errs.push('probe: việc đặt hôm nay không nằm trong "Hôm nay của tôi" (' + JSON.stringify(mdr) + ')');
      if (mdr.nGone !== 0) errs.push('probe: bấm bỏ My Day mà việc vẫn còn trong danh sách (' + JSON.stringify(mdr) + ')');
      if (mdr.md !== null) errs.push('probe: bấm bỏ My Day không ghi myDay = null (' + JSON.stringify(mdr) + ')');
      if (!mdr.skip) errs.push('probe: bấm bỏ My Day không ghi myDaySkip -> việc tự mọc lại (' + JSON.stringify(mdr) + ')');
      if (!mdr.stillToday) errs.push('probe: việc hạn hôm nay bị mất hẳn khỏi "Hôm nay" (' + JSON.stringify(mdr) + ')');
      if (!mdr.backTomorrow) errs.push('probe: hôm sau việc không tự hiện lại trong My Day (' + JSON.stringify(mdr) + ')');
