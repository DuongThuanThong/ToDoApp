...
const DEFAULT = () => ({
  ...
  settings: {
    ...
    soundDone: true, soundDoneFile: null,              // tiếng báo khi tick xong việc (null = tiếng mặc định của app)
...
function seedSmoke() {
  ...
  // Việc LẶP không có hạn mà rơi đúng hôm nay -> phải hiện ở CẢ "Hôm nay" lẫn "Hôm nay của tôi"
  const f = T({ title: 'Ôn từ vựng', duration: 20, repeat: { type: 'weekly', n: 1, days: [new Date().getDay()] } });
  fs.writeFileSync(DB_PATH, JSON.stringify({ folders: [], lists: [{ id: 'inbox', name: 'Inbox', folderId: null }, { id: uid(), name: 'Học tập', folderId: null }], tasks: [p, c1, c2, d, e, f], settings: { miniOn: true } }, null, 1));   // miniOn: để probe MINI-BOOT kiểm đúng đường "mở máy lên tự bật lại"
}
