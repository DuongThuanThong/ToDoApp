...
function view(tasks, name, now = new Date()) {
  ...
  switch (name) {
    // "Hôm nay của tôi" = CHỈ việc bạn tự chọn (nút ☀). Trước đây còn tự thêm việc đến hạn
    // hôm nay / quá hạn vào đây -> bấm bỏ khỏi My Day mà dòng vẫn nằm lại, nút như bị hỏng.
    case 'myday': return top.filter((t) => !t.done && t.myDay === ymd(now));
    case 'today': return top.filter((t) => !t.done && (isToday(t.due, now) || overdue(t, now)));
...
module.exports = {
  ...
  view, autoSchedule, stats, ymd, addDays, startOfDay, isToday, overdue,
