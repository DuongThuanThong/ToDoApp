...
function view(tasks, name, now = new Date()) {
  ...
  switch (name) {
    ...
    // "Chưa có hạn" chỉ còn việc MỘT LẦN chưa đặt hạn. Việc ĐỊNH KÌ (có `repeat`) đã có khung riêng
    // bên dưới — để lẫn vào đây thì khó phân biệt việc lỡ quên đặt hạn với việc cố ý làm hằng ngày.
    case 'nodate': return top.filter((t) => !t.done && !t.due && !t.repeat);
    // "Định kì" = KHO quản lý các việc lặp. Vì mỗi chuỗi lặp luôn có ĐÚNG MỘT bản chưa xong (bản vừa
    // sinh ra là kỳ kế tiếp), lọc `!done && repeat` cho ra đúng 1 dòng cho mỗi việc định kì — không
    // cần thêm mô hình "template" riêng. Bấm vào dòng để sửa lịch (thứ/ngày/giờ) bất cứ lúc nào.
    case 'recur': return top.filter((t) => !t.done && t.repeat);
