...
function toggleDone(tasks, id, done = null, now = new Date()) {
  ...
  if (t.done) {
    ...
    if (nd) { const clone = { ...t, id: uid(), done: false, doneAt: null, stage: 'todo', due: nd, parentId: t.parentId }; tasks.push(clone); }
  }
}
...
function view(tasks, name, now = new Date()) {
  ...
  switch (name) {
    case 'myday': return top.filter((t) => !t.done && (t.myDay === ymd(now) || isToday(t.due, now) || overdue(t, now)));
